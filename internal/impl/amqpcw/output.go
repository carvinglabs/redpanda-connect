...
func amqpCWOutputSpec() *service.ConfigSpec {
...
The fields 'key', 'exchange' and 'type' can be dynamically set using xref:configuration:interpolation.adoc#bloblang-queries[function interpolations].`).
		Fields(
			...
			service.NewStringField(accountHeaderField).
				Description("TBD").
				Default(""),
			service.NewStringField(eventHeaderField).
				Description("TBD").
				Default("CaptainWallet\\Integration\\Connect\\ConnectTaskRecordEvent"),
			service.NewBoolField(dedicatedHeaderField).
				Description("TBD").
				Default(false),
			service.NewBoolField(integrationConnectHeaderField).
				Description("TBD").
				Default(true),
			service.NewBoolField(integrationHeaderField).
				Description("TBD").
				Default(true),
			service.NewStringField(serializerHeaderField).
				Description("TBD").
				Default("php"),
			service.NewStringField(taskNameField).
				Description("TBD").
				Default(""),
		)
}
...
func init() {
	fmt.Println("registering amqp_cw output")
	err := service.RegisterOutput("amqp_cw", amqpCWOutputSpec(), func(conf *service.ParsedConfig, mgr *service.Resources) (service.Output, int, error) {
		maxInFlight, err := conf.FieldMaxInFlight()
		if err != nil {
			return nil, 0, err
		}
		w, err := amqpCWWriterFromParsed(conf, mgr)
...
type amqpCWWriter struct {
	...
	event              string
	integration        bool
	integrationConnect bool
	serializer         string
	taskName           string
}

func amqpCWWriterFromParsed(conf *service.ParsedConfig, mgr *service.Resources) (*amqpCWWriter, error) {
	a := amqpCWWriter{
		log: mgr.Logger(),
	}
	...
	if a.account, err = conf.FieldString(accountHeaderField); err != nil {
		return nil, err
	}
	if a.dedicated, err = conf.FieldBool(dedicatedHeaderField); err != nil {
		return nil, err
	}
	if a.event, err = conf.FieldString(eventHeaderField); err != nil {
		return nil, err
	}
	if a.serializer, err = conf.FieldString(serializerHeaderField); err != nil {
		return nil, err
	}
	if a.taskName, err = conf.FieldString(taskNameField); err != nil {
		return nil, err
	}
	if a.integration, err = conf.FieldBool(integrationHeaderField); err != nil {
		return nil, err
	}
	if a.integrationConnect, err = conf.FieldBool(integrationConnectHeaderField); err != nil {
		return nil, err
	}
...
func (a *amqpCWWriter) Connect(ctx context.Context) error {
	a.connLock.Lock()
...
func (a *amqpCWWriter) disconnect() error {
	a.connLock.Lock()
	defer a.connLock.Unlock()
...
func (a *amqpCWWriter) declareExchange(exchange string) error {
...
func (a *amqpCWWriter) Write(ctx context.Context, msg *service.Message) error {
	a.connLock.RLock()
	conn := a.conn
	amqpChan := a.amqpChan
	...
	*/
	...
	headers["account"] = a.account
	headers["dedicated"] = strconv.FormatBool(a.dedicated)
	headers["event"] = a.event
	headers["integration"] = strconv.FormatBool(a.integration)
	headers["integration-connect"] = strconv.FormatBool(a.integrationConnect)
	headers["serializer"] = a.serializer
...
func (a *amqpCWWriter) Close(context.Context) error {
	return a.disconnect()
}

// reDial connection to amqp with one or more fallback URLs.
func (a *amqpCWWriter) reDial(urls []string) (conn *amqp.Connection, err error) {
	for _, u := range urls {
		conn, err = a.dial(u)
		if err != nil {
...
func (a *amqpCWWriter) dial(amqpURL string) (conn *amqp.Connection, err error) {
