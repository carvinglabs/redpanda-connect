...
func amqpCWInputSpec() *service.ConfigSpec {
...
func init() {
	err := service.RegisterInput("amqp_0_9", amqpCWInputSpec(), func(conf *service.ParsedConfig, mgr *service.Resources) (service.Input, error) {
		return amqpCWReaderFromParsed(conf, mgr)
	})
	if err != nil {
		panic(err)
	}
...
type amqpCWBindingDeclare struct {
	exchange   string
	routingKey string
}
...
type amqpCWReader struct {
	...
	bindingDeclare []amqpCWBindingDeclare

	log *service.Logger
	m   sync.RWMutex
}

func amqpCWReaderFromParsed(conf *service.ParsedConfig, mgr *service.Resources) (*amqpCWReader, error) {
	a := amqpCWReader{
		log: mgr.Logger(),
	}
...
func (a *amqpCWReader) Connect(ctx context.Context) (err error) {
	a.m.Lock()
	defer a.m.Unlock()
...
func (a *amqpCWReader) disconnect() error {
	a.m.Lock()
	defer a.m.Unlock()
...
func (a *amqpCWReader) Read(ctx context.Context) (*service.Message, service.AckFunc, error) {
...
func (a *amqpCWReader) Close(ctx context.Context) error {
	return a.disconnect()
}

// reDial connection to amqp with one or more fallback URLs.
func (a *amqpCWReader) reDial(urls []string) (conn *amqp.Connection, err error) {
	for _, u := range urls {
		conn, err = a.dial(u)
		if err != nil {
...
func (a *amqpCWReader) dial(amqpURL string) (conn *amqp.Connection, err error) {
